...
struct SizeIterator {
  ...
  [[nodiscard]] constexpr bool operator!=(const SizeIterator& other) const noexcept {
    return loc != other.loc;
  }

  [[nodiscard]] constexpr const std::pair<std::size_t, std::size_t>& operator*() const noexcept {
    return loc;
  }
  [[nodiscard]] constexpr std::pair<std::size_t, std::size_t>& operator*() noexcept { return loc; }
};

[[nodiscard]] constexpr SizeIterator begin(const Size& t_s) noexcept { return SizeIterator{t_s}; }
...
constexpr auto opt_pow(const std::complex<ComplexType>& t_val, PowerType t_power) {
  if (t_power == static_cast<PowerType>(1.0)) {
    ...
    //    const auto a = std::real(t_val);
    //    const auto b = std::imag(t_val);
    //    return std::complex{ pow<4>(a) + pow<4>(b) - 6 * pow<2>(a) * pow<2>(b), 4 * pow<3>(a) * b
    //    - 4 * a * pow<3>(b) };
  } else {
...
constexpr auto get_color(const Point<PointType> t_point, const Point<CenterType> t_center,
                         const Size t_size, const ScaleType t_scale, std::size_t max_iteration,
                         const CenterType power, const bool do_abs) noexcept {
  const std::complex scaled{t_point.x / (t_size.width / t_scale) +
  ...
  const auto value =
      ((iteration + 1) -
       (std::log(std::log(std::abs(std::real(current) * std::imag(current))))) / std::log(power));
...
void set_pixel(sf::Image& img, const Point<PointType>& t_point, const Color<ColorType>& t_color) {
...
constexpr auto get_indicies() {
  ...
  std::generate(begin(indicies), end(indicies),
...
void run(Image<Width, Height>* img, const Settings* global_settings) {
  ...
  while (!settings.canceling) {
    ...
    if (cur_max_iterations <= max_max_iterations) {
      ...
      std::cout << cur_max_iterations << "\n";
      std::transform(std::execution::par_unseq, begin(indicies), end(indicies),
                     begin(localImg->colors), [=](const auto& location) {
                       return get_color(Point{location.first, location.second}, settings.center,
                                        size, settings.scale, cur_max_iterations, settings.power,
                                        settings.do_abs);
                     });
      ...
      *img = *localImg;
      ...
      if (cur_max_iterations + max_iteration_increment >= max_max_iterations) {
        std::cout << "Max iterations rendered in "
